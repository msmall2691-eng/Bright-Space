"""Multi-tenancy MT-2: jobs are scoped to the caller's workspace.

Same pattern as clients/properties: the master-API-key test caller resolves
to the default org, so a job planted in another org must be invisible to the
list/detail endpoints and 404 on update/delete; legacy NULL-org rows stay
visible.

(The Visit-scope assertions that used to live here were retired with the
Job/Visit unification — occurrences are Jobs now.)
"""
import uuid
...
def test_other_org_job_is_invisible():
    db = SessionLocal()
    c, j = _seed(db, OTHER_ORG)
    try:
...
def test_legacy_null_org_job_stays_visible():
    db = SessionLocal()
    c, j = _seed(db, None)  # pre-tenancy rows
    try:
