...
def _run_push_open_shifts(run_id: str, start, end) -> None:
    ...
    from integrations.connecteam import (
        ConnecteamAuthError, build_shift_payload, create_shifts_sync, match_job_id,
    )
    ...
    try:
        ...
        # Eager-load property/client so the per-job Connecteam-Job-name match
        # below doesn't N+1 query the DB once per job in the range.
        jobs = (
            db.query(Job)
