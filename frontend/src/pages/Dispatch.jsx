    ...
    <div className="p-4 sm:p-6 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400">Push scheduled jobs as shifts to your cleaners.</p>
        </div>
        <button onClick={load}
          className="flex items-center gap-2 bg-gray-100 hover:bg-gray-200 border border-gray-200 px-3 py-2 rounded-lg text-sm transition-colors">
      ...
      {pending.length > 0 && (
        <div className="mb-6">
          <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-3">Ready to Dispatch</h3>
          <div className="space-y-3">
            {pending.map(j => (
              <div key={j.id} className="bg-white border border-gray-200 rounded-xl p-5 hover:border-gray-300 transition-colors">
                <div className="flex items-start justify-between">
                  <div className="min-w-0">
                    ...
                    {j.cleaner_ids?.length > 0 && (
                      <div className="flex items-center gap-1.5 mt-2">
                        ...
                        <span className="text-xs text-gray-500">{j.cleaner_ids.map(empName).join(', ')}</span>
                      </div>
                    )}
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-gray-900 hover:bg-gray-800 text-white disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed px-4 py-2 rounded-lg text-sm font-medium transition-colors shrink-0 ml-4"
                  >
      ...
      {dispatched.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-3">Already Dispatched</h3>
          <div className="space-y-2">
            {dispatched.map(j => (
              <div key={j.id} className="bg-gray-50 border border-gray-100 rounded-xl p-4">
                <div className="flex items-center justify-between">
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-gray-700">{j.title}</div>
                    <div className="text-xs text-gray-400 mt-0.5">{j.scheduled_date} · {j.start_time}–{j.end_time}</div>
                  </div>
      ...
      {jobs.length === 0 && !loading && (
        <div className="text-center py-16">
          ...
          <div className="text-gray-500 font-medium mb-1">No scheduled jobs to dispatch</div>
          <p className="text-sm text-gray-400 mb-4">Schedule jobs first, then dispatch them to your field team.</p>
