...
export default function OpportunityLinker({ clientId, itemType, itemId, itemName, currentOpportunityId, onLinked }) {
  ...
  return (
    <div>
      ...
      {/* Modal — shared ui/Modal shell (focus-trap / Esc / scroll-lock); not
          dismissable mid-save so a link/unlink in flight can't be interrupted. */}
      <Modal
        open={showModal}
        onClose={() => setShowModal(false)}
        dismissable={!saving}
        maxWidth="lg"
        title={`Link ${itemType} to Opportunity`}
      >
