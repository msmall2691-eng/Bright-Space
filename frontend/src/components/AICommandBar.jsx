  ...
  if (!open) {
    return (
      <button
        ...
        className="no-print lg:hidden fixed bottom-[4.75rem] right-4 z-50 flex items-center gap-2 px-3.5 py-2.5 bg-panel text-ink-2 rounded-xl shadow-lg border border-hairline hover:border-hairline-2 hover:shadow-xl transition-all group"
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-hairline">
          ...
          {loading ? (
            ...
            <button onClick={ask} className="p-1 text-ink-3 hover:text-ink-2">
        ...
        {/* Answer */}
        {answer && (
          <div className="px-5 py-4 border-b border-hairline max-h-60 overflow-y-auto">
            <p className="text-sm text-ink-2 whitespace-pre-wrap leading-relaxed">{answer.answer}</p>
          </div>
        )}
        ...
        {followups && followups.total > 0 && !answer && (
          <div className="px-5 py-3 border-b border-hairline">
            <p className="text-xs font-semibold text-ink-3 uppercase tracking-wide mb-2">Needs Attention</p>
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {followups.followups.slice(0, 5).map((f, i) => (
                <div key={i} className="flex items-start gap-2 text-sm">
                  ...
                  <div>
                    <span className="font-medium text-ink-2">{f.title}</span>
                    <p className="text-xs text-ink-3">{f.detail}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {!answer && (
          <div className="px-5 py-3">
            <p className="text-xs text-ink-3 mb-2">Try asking:</p>
            <div className="flex flex-wrap gap-2">
              {["How many jobs this week?", "Any unanswered messages?", "What's my revenue outlook?", "Who are my newest leads?"].map(q => (
                <button
                  ...
                  className="text-xs px-3 py-1.5 bg-bg hover:bg-bg-2 text-ink-2 rounded-full border border-hairline transition-colors"
                >
                  {q}
                </button>
              ))}
