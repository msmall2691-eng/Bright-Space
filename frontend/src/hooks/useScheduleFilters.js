...
// my schedule" chips. unassignedOnly/noGcalOnly are deliberately NOT
// persisted: those are one-shot "jump to the problem list" actions
// (triggered from a dashboard follow-up or a health-strip counter),
...
export function useScheduleFilters({ visits, jobs, properties, viewMode, dateStr }) {
  ...
  // "no-Google" quick filter — driven both by a URL param (deep links from
  // the dashboard's follow-up cards) and by the clickable counters in
  // ScheduleHealthStrip. Audit finding: those counters were dead numbers;
  // now they narrow the list to the offenders.
  ...
  // One-shot sync from URL ?filter=unassigned|no_gcal so a click on a
  // dashboard follow-up lands the operator on the queue with the filter
  // pre-applied. Runs once on mount; user tweaks after that stick.
  ...
  const filteredVisits = useMemo(() => {
    ...
    return visits
      .filter(v => {
        ...
        // "Not on Google" — only against active visits, and gcal_event_id
        // resolves through the linked job (the id lives on Job, not Visit,
        // same as scheduleStats above).
        if (noGcalOnly && (v.status === 'cancelled' || v.status === 'completed')) return false
        if (noGcalOnly) {
  ...
  }, [visits, selectedPropertyType, selectedStatus, unassignedOnly, noGcalOnly, jobs, properties])
